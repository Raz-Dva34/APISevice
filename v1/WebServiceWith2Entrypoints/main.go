...
func handlePostItem(w http.ResponseWriter, r *http.Request, store *Store) {
	...
	if caption == "" || weightStr == "" || numberStr == "" {
	...
	if weight < 0 || number < 0 {
		http.Error(w, "Вес и количество должны быть положительными", http.StatusBadRequest)
		return
	}

	newItem := Item{Caption: caption, Weight: float32(weight), Number: number}
